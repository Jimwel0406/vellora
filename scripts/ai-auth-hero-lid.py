...
PROMPT = (
    "fix this image: give the candle a clean neat flat wooden lid with a smooth surface, "
    "and make the trio of lip balms smaller so it is in proportion with the other items in the bag. "
    "Keep everything else exactly the same: the same kraft shopping bag, the same terracotta ceramic plant pot, "
    "the same rolled woven throw blanket, same cream linen background, same lighting, "
    "same square centered composition. Photorealistic, high detail. No text, no logo, no watermark."
)
ASPECT = "1:1"
TRIES = 4

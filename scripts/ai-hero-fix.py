...
REF = "public/hero-curation.jpg"
OUT = "public/hero-curation.jpg"
PROMPT = "remove the bamboo toothbrush from this image completely, keep everything else exactly the same: same soy candle, same ceramic pot, same blanket, same wraps, same background, same lighting, same layout. Photorealistic, high detail, portrait composition"
TRIES = 4
...
if __name__ == "__main__":
    ...
    for n in range(TRIES):
        img, err = gen_edit(ref, PROMPT, "3:4")
        if not img:
            print(f"try {n+1}: {err}")
            continue

...
# same product visual identity; only angle/lighting/framing varies
PRODUCTS = [
...
VARIANTS = [
    (
        "three-quarter side angle view",
        "warm cream studio background, soft diffused lighting, gentle shadow, photorealistic, high detail, square composition",
    ),
    (
        "close-up detail macro shot",
        "warm cream studio background, slightly warmer golden lighting, subtle shadow, photorealistic, high detail, square composition",
    ),
]
...
if __name__ == "__main__":
    ...
    for slug, base_prompt in PRODUCTS:
        if only and only not in slug:
            continue
        for idx, (angle, style) in enumerate(VARIANTS, start=1):
            out = os.path.join("public", "products", f"{slug}-{idx}.jpg")
            if os.path.exists(out):
                print(f"SKIP {out} (exists)")
                continue
            prompt = f"{base_prompt}, {angle}. {style}"
